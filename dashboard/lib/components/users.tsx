...
function getUsersByTime(data: Data) {
    ...
    // Track first time we see each user
    for (const row of sortedData) {
        ...
        if (!usersByTime.has(timeKey)) {
            usersByTime.set(timeKey, 0);
        }
        usersByTime.set(timeKey, usersByTime.get(timeKey) + 1);
    }
    
    // Convert to array of points for graphing
    const buckets = Array.from(usersByTime.entries())
...
export default function Users({ data }: {data: Data}) {
    ...
    useEffect(() => {
        ...
        setUserTrend(getUsersByTime(data));
    }, [data]);
    ...
    const renderBackgroundGraph = () => {
        ...
        const maxCount = Math.max(...userTrend.map(b => b.count));
